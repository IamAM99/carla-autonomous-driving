...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            ...
            self.destroy()
            self = self.world.try_spawn_actor(blueprint, spawn_point)
...
def game_loop(args):
    ...
    try:
        ...
        route_points = route.get_transforms(point_distance=4)
        reward = RouteReward(route_points)
        draw_locations(world.world, route_points)
        ...
        try:
            if args.create_new_route:
                ...
                pitch_ = [w.transform.rotation.pitch for w in waypoints]
                yaw_ = [w.transform.rotation.yaw for w in waypoints]
                roll_ = [w.transform.rotation.roll for w in waypoints]
                ...
                route_vars = [
                    ...
                    for x, y, z, pitch, yaw, roll \
                    in zip(x_, y_, z_, pitch_, yaw_, roll_)
                ]
                with open("route.pickle", 'wb') as handle:
                    pickle.dump(route_vars, handle, protocol=pickle.HIGHEST_PROTOCOL)
        except RuntimeError:
            print("couldn't save")
